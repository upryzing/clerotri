...
const ChannelSettings = observer(
    ...
    return (
      <>
        ...
        {server.orderedRoles.map(r => (
          <SettingsEntry key={`channel-settings-${channel._id}-perms-${r.id}`}>
            <View style={{flex: 1}}>
              <View>
                <Text
    ...
    return (
      <>
        ...
        {section!.subsection !== undefined && channel ? (
          <ChannelSettings channel={channel} server={server} />
        ) : (
          <ChannelSettingsChannelList
            server={server}
